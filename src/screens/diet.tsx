...
import {Racao_aplicadaType} from '@/types/racao_aplicado';

const fetchRacao = () => {
    return axios.get<RacaoType[]>('http://localhost:6754/racao').then(({ data }) => data);
}
                        ...
                        <Select disabled={!brincoRead} placeholder="Definir Ração" style={{ width: 550 }}>
                            <option value=""></option>
                            {
                                ...
                                getRacao.data.map(({ id_racao, tipo_racao, nome_racao }) => {
                                    return <option key={id_racao} value={id_racao}>{ `${nome_racao}/${tipo_racao}` }</option>
                                })
                            }
                        </Select>
                    </Form.Item>
